import os, time, math, base64, re, aiohttp, aiofiles, asyncio, requests, shutil
...
# --- GLOBAL VARIABLES ---
user_data, batch_data, download_queue, user_modes, REPLACE_DICT = {}, {}, {}, {}, {}
ACTIVE_TASKS, MAX_TASK_LIMIT = 0, 5
...
async def watermark_cmd(client, message):
    await message.reply_text("🖼 <b>Ab Photo Bhejein!</b> (Main use Thumbnail ya Watermark bana dunga)")
...
# --- PHOTO HANDLER (Watermark Priority Fix) ---
@app.on_message(filters.private & filters.photo)
async def handle_photos(client, message):
    # Ye handler sabse pehle chalega agar photo aayi
...
async def save_callback(client, callback):
    ...
    try:
        ...
        else: await callback.message.edit_text("❌ Error: Photo not found.")
...
async def img_type_callback(client, callback):
    # (Same Logic as before, just short for brevity)
...
async def batch_done(client, message):
    ...
    if uid in batch_data and batch_data[uid]['files']:
        ...
        await message.reply_text(f"✅ <b>{len(batch_data[uid]['files'])} Files.</b> Send Series Name:")
    else: await message.reply_text("⚠️ No files!")
...
async def batch_process(client, callback):
    ...
    for idx, msg in enumerate(files_list):
        ...
        start = time.time()
        if mode == 'video':
            await client.send_video(uid, dl, caption=caption, thumb=thumb_path, duration=dur, width=w, height=h, progress=progress, progress_args=(status, start, f"📤 UL {idx+1}/{total}"))
        else:
            await client.send_document(uid, dl, caption=caption, thumb=thumb_path, progress=progress, progress_args=(status, start, f"📤 UL {idx+1}/{total}"))
        os.remove(dl)
...
async def handle_files(client, message):
    ...
    # Check mime type again just in case (Image fix)
    mime = getattr(message.document or message.video, "mime_type", "")
    if "image" in mime:
        return await handle_photos(client, message)
...
async def single_mode(client, callback):
    ...
    # Note: Text handler needs logic to catch this reply if you want custom name for single file. 
    # For now, simplistic flow assumed.
