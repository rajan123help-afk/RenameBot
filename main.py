import os, time, math, base64, re, asyncio, requests, shutil, html, aiofiles, aiohttp
...
LOG_CHANNEL = "@filmyflip_screenshots" # 👈 New Channel Config
...
def clean_filename(name):
    for old_word, new_word in cleaner_dict.items():
        if old_word in name: name = name.replace(old_word, new_word)
    return name.strip()
    # 🔥 NEW: STRICT LOGIC (Ignores 2025/1080p)
...
def get_fancy_caption(filename, filesize, duration=0):
    ...
    # Using Strict Logic for cleaner captions
...
async def add_clean(client, message):
    ...
    replace = " Filmy Flip Hub"
    if len(message.command) > 2: replace = " ".join(message.command[2:])
    cleaner_dict[word] = replace
    await message.reply_text(f"✅ <b>Set:</b> {word} ➡️ {replace}")
...
async def type_callback(client, callback):
    # (Keeping your original TMDB logic logic)
    try:
...
async def num_callback(client, callback):
    # (Your original TMDB image sending logic)
    try:
        ...
        # ... (Fetching pool logic same as your code)
...
async def url_handler(client, message):
    ...
    # (Your Link Mode Check)
    if user_modes.get(uid) == "link":
        code = text.split("/")[-1] if "t.me/" in text else text
        await message.reply_text(f"🔗 <code>{BLOGGER_URL}?data={base64.b64encode(code.encode()).decode()}</code>")
        return
...
async def dl_process(client, callback):
    ...
    try:
        ...
        # Watermark Thumbnail Logic (Your Original)
        if not thumb_path and os.path.exists(f"watermarks/{uid}.png"):
...
async def batch_run(client, callback):
    # (Your existing batch run logic - omitted for brevity but should be kept)
    pass
